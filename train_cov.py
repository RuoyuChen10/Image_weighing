...
def forward(x, keep_prob):
    ## convl layer ##
    W_conv1 = weight_variable([3, 3, 3, 32], weight_name = "conv1")  # kernel 3*3, channel is 3
    b_conv1 = bias_variable([32], bias_name = "cb1")
    h_conv1 = tf.nn.relu(conv2d(x, W_conv1) + b_conv1)  # output size 212*48*3
    h_pool1 = max_pool_2x2(h_conv1)  # output size 106*24*3

    ## conv2 layer ##
    W_conv2 = weight_variable([3, 3, 32, 64], weight_name = "conv2")  # kernel 3*3, in size 3, out size 5
    b_conv2 = bias_variable([64], bias_name = "cb2")
    h_conv2 = tf.nn.relu(conv2d(h_pool1, W_conv2) + b_conv2)  # output size 104*22*5
    h_pool2 = max_pool_2x2(h_conv2)  # output size 52*11*5

    ## funcl layer ##
    W_fc1 = weight_variable([36608, 1024], weight_name = "fc1")
    b_fc1 = bias_variable([1024], bias_name = "fb1")

    # [n_samples,7,7,64]->>[n_samples, 7*7*64]
    h_pool2_flat = tf.reshape(h_pool2, [-1, 36608])
...
def backward(datasets, label, test_data, test_label):
    X = tf.placeholder(tf.float32, [None, 214, 50, 3], name = "X")
...
def main():
    datasets, label, test_data, test_label = reload_all_data_2D()
