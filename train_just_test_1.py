...
def forward(x, keep_prob):
    ...
    ## conv4 layer ##
    W_conv4 = weight_variable([3, 3, 16, 32], weight_name="conv4")  # kernel 3*3, in size 3, out size 5
    b_conv4 = bias_variable([32], bias_name="cb4")
    h_conv4 = tf.nn.softplus(conv2d(h_pool3, W_conv4) + b_conv4)  # output size 27*6*32
    h_pool4 = max_pool_2x2(h_conv4)  # output size 13*3*32

    ## conv5 layer ##
    W_conv5 = weight_variable([3, 3, 32, 64], weight_name="conv4")  # kernel 3*3, in size 3, out size 5
    b_conv5 = bias_variable([64], bias_name="cb4")
    h_conv5 = tf.nn.elu(conv2d(h_pool4, W_conv5) + b_conv5)  # output size 11*1*64

    ## funcl layer ##
    W_fc1 = weight_variable([11 * 1 * 64, 32], weight_name="fc1")
    b_fc1 = bias_variable([32], bias_name="fb1")

    # [n_samples,7,7,64]->>[n_samples, 7*7*64]
    h_pool5_flat = tf.reshape(h_conv5, [-1, 11 * 1 * 64])
    h_fc1 = tf.nn.softplus(tf.matmul(h_pool5_flat, W_fc1) + b_fc1)
...
def backward():
    datasets, label, test_data, test_label = reload_choose_duck_R_no_background_just_1(1)
    ...
    with tf.Session() as sess:
        ...
        for i in range(STEPS):
            start = (i * BATCH_SIZE) % 3920  # duck3 1930  duck7
            ...
            if i % 100 == 0:
                ...
                f.write(
                    "After %d training step(s), loss_mse on train data is %g, loss_mse on val data is %g, min_loss is %g\n" % (
